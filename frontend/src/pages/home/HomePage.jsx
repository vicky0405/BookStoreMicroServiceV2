...
const banners = [
  {
    id: 1,
    // 2. Gán biến vừa import vào đây (KHÔNG để trong dấu ngoặc kép "")
    image: bannerImg1,
  },
  {
    id: 2,
  ...
  return (
    <div className="banner-slider">
      ...
      <div className="banner-controls">
        ...
        <button onClick={() => setCurrent((current + 1) % banners.length)}>
          &gt;
        </button>
      </div>
  ...
  return (
    <section className="homepage-section">
      ...
      <div className="products-slider">
        {books.map((book) => {
          const hasDiscount =
            Number(book.originalPrice) > Number(book.discountedPrice);
          return (
            <div className="product-card" key={book.id}>
              ...
              <div className="card-price-wrap">
                <span className="price-discount">
                  {formatCurrency(book.discountedPrice ?? book.price)}
                </span>
                {hasDiscount && (
            ...
            <div className="promo-header">
              ...
              <span className="promo-discount">
                {promo.type === "percent" || promo.discountType === "percent"
                  ...
                  : `-${
                      promo.discount
                        ? Number(promo.discount).toLocaleString("vi-VN")
                        : ""
                    }đ`}
              </span>
            </div>
...
function HomePage() {
  ...
  const latestCards = (latestBooks || []).map((book) => {
    ...
    const discounted =
      pv.discounted_price ?? book.discounted_price ?? book.price;
    const { original: o, discounted: d } = normalizePricing(
      original,
      discounted
    );
    return {
